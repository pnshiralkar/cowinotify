...
from config import PINCODES_PHONE_NUMBERS, RESEND_SMS_INTERVAL, MOCK_SMS, ERROR_SMS_INTERVAL, ERROR_SMS_NUMBERS
    ...
    for center in resp.json()['centers']:
        ...
        for session in center['sessions']:
            if session['available_capacity'] > 0:
                ...
                print(
                    f"{center['name']} - {session['date']} : Min Age = {session['min_age_limit']}, Capacity = {session['available_capacity']}")
        if available:
            message += f"\n{center['name']}:\n"
